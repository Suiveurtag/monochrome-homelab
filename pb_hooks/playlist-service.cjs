...
function refreshIdentities(app, record) {
    ...
    record.set('identities', ids.map((id) => {
        const user = app.findRecordById('users', id);
        let name = user.getString('name') || 'Member';
        const profiles = app.findRecordsByFilter('DB_users', 'firebase_id = {:id}', '', 1, 0, { id });
        if (profiles.length) name = profiles[0].getString('display_name') || profiles[0].getString('username') || name;
        return { id, name, role: id === record.getString('owner') ? 'owner' : 'collaborator' };
    }));
}

function syncPublic(app, record, deleted) {
    const mirrors = app.findRecordsByFilter('public_playlists', 'uuid = {:uuid}', '', 1, 0, { uuid: record.getString('uuid') });
    ...
    if (mirrors.length && mirror.getString('uid') !== record.getString('owner')) throw new ForbiddenError('Playlist belongs to another account.');
    const name = record.getString('name');
    const cover = record.getString('cover');
    const description = record.getString('description');
    const values = { uuid: record.getString('uuid'), uid: record.getString('owner'), firebase_id: record.getString('owner'), title: name, name, playlist_name: name, image: cover, cover, playlist_cover: cover, description, tracks: json(record, 'tracks', []), isPublic: true, data: { title: name, cover, description } };
...
function create(e) {
    ...
    e.app.runInTransaction((app) => {
        ...
        if (!body.uuid || !body.name || String(body.uuid).length > 128) throw new BadRequestError('Playlist name and ID are required.');
        const existing = app.findRecordsByFilter('shared_playlists', 'uuid = {:uuid}', '', 1, 0, { uuid: body.uuid });
        if (existing.length) {
            if (existing[0].getString('owner') !== e.auth.id) throw new ForbiddenError('Playlist belongs to another account.');
            result = existing[0];
            return;
        }
        const publicRecords = app.findRecordsByFilter('public_playlists', 'uuid = {:uuid}', '', 1, 0, { uuid: body.uuid });
        if (publicRecords.length && publicRecords[0].getString('uid') !== e.auth.id) throw new ForbiddenError('Playlist belongs to another account.');
        const record = new Record(app.findCollectionByNameOrId('shared_playlists'));
        const values = { uuid: body.uuid, owner: e.auth.id, members: [], name: body.name, cover: body.cover || '', description: body.description || '', isPublic: !!body.isPublic, revision: 1 };
        Object.keys(values).forEach((field) => record.set(field, values[field]));
        try { record.set('tracks', applyTrackOperation([], { type: 'add', tracks: body.tracks || [] }, Date.now())); }
...
function mutate(e) {
    ...
    e.app.runInTransaction((app) => {
        ...
        if (!owner && !members.includes(e.auth.id)) throw new ForbiddenError('You no longer have access to this playlist.');
        if (body.revision !== record.getInt('revision')) throw new ApiError(409, 'Playlist changed. Refresh and try again.');
        if (['add', 'remove', 'reorder'].includes(operation.type)) {
            try { record.set('tracks', applyTrackOperation(json(record, 'tracks', []), operation, Date.now())); }
            catch (error) { throw new BadRequestError(error.message); }
        } else if (operation.type === 'add-member') {
            if (!owner) throw new ForbiddenError('Only the owner can add collaborators.');
            const query = String(operation.username || '').trim().replace(/^@/, '');
            if (!query) throw new BadRequestError('Enter a username or account ID.');
            const profiles = app.findRecordsByFilter('DB_users', 'username = {:query} || firebase_id = {:query}', '', 2, 0, { query });
            let id = profiles.length === 1 ? profiles[0].getString('firebase_id') : '';
            if (!id && /^[a-z0-9]{15}$/.test(query)) {
                try { id = app.findRecordById('users', query).id; } catch (_) { /* Missing user is reported below. */ }
            }
            ...
            record.set('members', members.filter((member) => member !== id));
        ...
        app.save(record);
        syncPublic(app, record, false);
        result = operation.type === 'leave' || (operation.type === 'remove-member' && operation.userId === e.auth.id) ? { left: true, uuid: record.getString('uuid') } : record;
    });
    return result;
}
