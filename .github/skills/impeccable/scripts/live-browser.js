...
(function () {
    ...
    const LIVE_UI_SURFACES = [
        ...
        },
        {
            ...
            ids: [PREFIX + '-annot', PREFIX + '-annot-svg', PREFIX + '-annot-pins', PREFIX + '-annot-clear'],
        },
    ...
    function buildCyclingRow() {
        ...
        if (hasParams || paramsPending) {
            ...
            if (hasParams) {
                ...
                tune.title =
                    'Tune this variant (' + visParams.length + ' knob' + (visParams.length === 1 ? '' : 's') + ')';
    ...
    function documentRefSegment(el) {
        const tag = el.tagName.toLowerCase();
        return (
            tag + documentRefIdSuffix(el) + documentRefClassSuffix(el) + ':nth-of-type(' + indexAmongSameTag(el) + ')'
        );
    }
    ...
    function updateManualApplyRepairState(repair, phase) {
        const count =
            parseInt(pendingPillEl?.dataset.count || '0', 10) || Number(readStoredManualApplyState()?.count) || 0;
        if (count <= 0) return;
    ...
    function mountedParameterCount() {
        ...
        if (!wrapper) return 0;
        return [
            ...wrapper.querySelectorAll('[data-impeccable-variant]:not([data-impeccable-variant="original"])'),
        ...
        for (const entry of contract) {
            if (entry.kind === 'collection' && entry.item && entry.item.rootTag) {
                ...
                if (entry.probe && entry.probe.tag) {
                    const selector =
                        entry.probe.tag + (entry.probe.classes || []).map((c) => '.' + cssEscapeIdent(c)).join('');
                    try {
