...
(function () {
    ...
    function isCardLikeDOM(el) {
        ...
        const hasShadow =
            (style.boxShadow && style.boxShadow !== 'none') || /\bshadow(?:-sm|-md|-lg|-xl|-2xl)?\b/.test(cls);
    ...
    function positionedChildEscapesClip(el, child, clipX, clipY) {
        ...
        return Boolean(
            (clipX &&
                (childRect.left < parentRect.left - threshold || childRect.right > parentRect.right + threshold)) ||
    ...
    function checkElementTextOverflowDOM(el) {
        ...
        if (el.clientWidth > 0 && delta >= 16) {
            return [
                { id: 'text-overflow', snippet: `${classSelector(el)} overflows its box by ${Math.round(delta)}px` },
            ];
        }
  ...
  `;
        ...
        async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
            ...
            const stack =
                typeof document.elementsFromPoint === 'function' ? document.elementsFromPoint(point.x, point.y) : [];
            ...
            return {
                status: 'unresolved',
                reason:
                    [...new Set(unresolved.filter(Boolean))].slice(0, 3).join(', ') || 'no readable visual background',
            };
        }
        ...
        function scheduleLazyVisualContrast(groupMap, analyses, options = {}, runtime = {}) {
            ...
            const unresolved = (analyses || []).filter(
                (result) =>
                    result?.status === 'unresolved' && result.reason === 'text outside viewport' && result.selector
            );
