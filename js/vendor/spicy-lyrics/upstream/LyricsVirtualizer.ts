...
import { Maid } from './runtime/Maid.ts';
...
export class LyricsVirtualizer {
    ...
    init(scrollEl: HTMLElement, virtualContainer: HTMLElement, lineElements: HTMLElement[]): void {
        ...
        this._maid.Give(() => document.removeEventListener('visibilitychange', _handleVisibilityRestore));
    }
    ...
    private _scrollToIndexWithRetry(
        ...
        const cached = v.measurementsCache[index] as { start: number; end: number; size: number } | undefined;
        ...
        if (retry < LyricsVirtualizer._MAX_SCROLL_RETRIES) {
            this._scrollVerifyRAF = requestAnimationFrame(() => {
                ...
                if (!targetMounted || drift >= 1 || wasClamped) {
                    this._scrollToIndexWithRetry(index, align, instant, padding, retry + 1, observedScrollTop);
                } else {
                    this._setConverging(false);
                }
            });
