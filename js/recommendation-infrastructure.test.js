...
test('Smart Random retains duplicates while postponing repeated tracks and recent artists', () => {
    ...
    const ranked = smartRandom(tracks, {
        random: () => 0,
        now: 1000,
...
test('Smart Random remains unpredictable with the same history', () => {
    ...
    expect(smartRandom(tracks, { random: () => 0 }).map((t) => t.id)).not.toEqual(
        smartRandom(tracks, { random: () => 0.99 }).map((t) => t.id)
    );
});
...
test('ranking responds to behavior, recency and discovery without an embedding adapter', () => {
    ...
    expect(score({ playCount: 4, earlySkipCount: 4, lastPlayed: now - 180 * DAY })).toBeGreaterThan(
        score({ playCount: 4, earlySkipCount: 4, lastPlayed: now - 3 * DAY })
    );
...
test('ranking spaces artists across the previous session and rejects unavailable or duplicate candidates', () => {
    const tracks = [
        ...
        song('b', 'b'),
        song('b', 'b'),
        { ...song('hidden'), isUnavailable: true },
        { ...song('video'), type: 'video' },
    ];
    const ranked = rankTracks(tracks, {
        exploration: 0,
        profile: { recent: [{ id: 'previous', artists: ['a'], at: Date.now() }] },
        excludeIds: ['a2'],
    });
...
test('metadata accepts string artists without assigning a compilation album artist to credited tracks', () => {
    expect(artistsOf({ artists: ['Artist'], album: { artist: { id: 'various', name: 'Various Artists' } } })).toEqual([
        { id: 'artist', name: 'Artist' },
    ]);
    expect(artistsOf({ album: { artist: { id: 'album-artist', name: 'Album Artist' } } })).toEqual([
...
function serviceFixture(overrides = {}) {
    ...
    return {
        ...
        service: new RecommendationService({
            ...
            favorites: async () => [],
            embeddings: null,
            shouldHideTrack: (track) => track.blocked,
            ...overrides,
        }),
    };
}
...
test('service applies favorites/content blocking and accepts a synchronous embedding adapter', async () => {
    const { service } = serviceFixture({
        favorites: async () => [song('liked')],
        embeddings: { getMany: () => new Map() },
    });
    const ranked = await service.rank([song('other'), song('liked'), { ...song('blocked'), blocked: true }], {
        exploration: 0,
    });
...
test('service discards pending recommendations when the authenticated account changes', async () => {
    ...
    const { service, tracker } = serviceFixture({
        favorites: () =>
            new Promise((resolve) => {
...
test('persisted embeddings retain model/version identity and overwrite only the matching record', async () => {
    ...
    try {
        ...
        await expect(
            adapter.putMany([{ trackId: 'bad', model: 'mert', version: '1', vector: [1, NaN] }])
        ).rejects.toThrow();
