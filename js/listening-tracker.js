...
export class ListeningTracker {
    ...
    get accountKey() { return this._storageKey; }

    _empty() { return { tracks: {}, artists: {}, genres: {}, albums: {}, likes: {}, recent: [], version: 2 }; }
    _load() {
        ...
        try {
            ...
            for (const key of ['tracks', 'artists', 'genres', 'albums', 'likes']) {
                if (!this._data[key] || typeof this._data[key] !== 'object' || Array.isArray(this._data[key])) this._data[key] = {};
            }
    ...
    _save() {
        try {
            ...
            for (const [key, limit] of [['tracks', 2000], ['artists', 500], ['albums', 500], ['genres', 200], ['likes', 3000]]) {
                const entries = Object.entries(data[key]);
                if (entries.length > limit) {
                    entries.sort((a, b) => (b[1].updatedAt || b[1].lastPlayed || 0) - (a[1].updatedAt || a[1].lastPlayed || 0));
                    data[key] = Object.fromEntries(entries.slice(0, limit));
                }
            }
            data.recent = data.recent.slice(-200);
            localStorage.setItem(this._storageKey, JSON.stringify({ ...data, pendingSession: this._sessionSnapshot() }));
        } catch (error) { console.warn('ListeningTracker: save failed', error); }
    }
    _flush() {
        if (this._flushTimer) return;
        this._flushTimer = setTimeout(() => { this._flushTimer = null; this._save(); }, 2000);
    }
    ...
    onTimeUpdate(currentTime, duration, { playbackRate = 1 } = {}) {
        ...
        if (delta > 0 && delta <= Math.max(5, elapsed * (Number(playbackRate) || 1) + 0.5)) this._accumulatedPlayTime += delta;
    ...
    onTrackEnd() { this._finalizeCurrent(false); }
    onSkip() { this._finalizeCurrent(true); }
    onSessionInterrupted() { this._finalizeCurrent('interrupted'); }

    _sessionSnapshot() {
        return this._currentTrack ? { track: this._currentTrack, playTime: this._accumulatedPlayTime,
            duration: this._trackDuration, at: Date.now() } : null;
    }

    _finalizeCurrent(wasSkipped = false) {
        if (!this._currentTrackId || this._playStartTime === null) return;
        const session = this._sessionSnapshot();
        this._recordSession(session, wasSkipped === true ? 'skipped' : wasSkipped === 'interrupted' ? 'interrupted' : 'ended');
        this._currentTrackId = null;
        this._currentTrack = null;
        this._playStartTime = null;
        this._accumulatedPlayTime = 0;
    ...
    _recordSession({ track, playTime, duration, at }, reason) {
        ...
        if (skipped) {
            signal.skipCount++;
            if (ratio < 0.3 || (playTime < 30 && ratio < 0.7)) { signal.earlySkipCount++; weight = -1; }
            else { signal.lateSkipCount++; weight = ratio >= 0.7 ? 0.15 : -0.2; }
        } else if (ratio >= 0.9) { signal.completionCount++; }
        if (replay) { signal.replayCount++; weight += 0.7; }
    ...
    _trackEntry(track) {
        ...
        const initial = { playCount: 0, skipCount: 0, totalPlayTime: 0, completionCount: 0, lastPlayed: 0,
            avgCompletionRatio: 0, earlySkipCount: 0, lateSkipCount: 0, replayCount: 0,
            likeCount: 0, playlistAddCount: 0, searchCount: 0 };
        const previous = data.tracks[id] || {};
        data.tracks[id] = { ...initial, ...previous, earlySkipCount: previous.earlySkipCount ?? previous.skipCount ?? 0,
    ...
    _affinity(track, weight, playTime = 0, skipped = false, now = Date.now()) {
        ...
        const bump = (collection, id, name = '') => {
            ...
            data[collection][id] = { ...previous, name,
                ...
                totalPlayTime: previous.totalPlayTime + playTime, updatedAt: now };
        };
    ...
    recordSignal(kind, item, { type = 'track', added = true } = {}) {
        if (!item?.id || !['track', 'artist', 'album'].includes(type) || item.isPodcast || item.type === 'video') return;
        const weight = { like: added ? 2 : -2, 'playlist-add': 1, search: 0.25 }[kind];
        if (weight === undefined) return;
        if (kind === 'like') {
            const data = this._load();
            const key = `${type}:${item.id}`;
            const previous = data.likes[key]?.added ?? (type === 'track' && Boolean(data.tracks[String(item.id)]?.likeCount));
            if (previous === Boolean(added)) return;
            data.likes[key] = { added: Boolean(added), updatedAt: Date.now() };
        }
        const track = type === 'artist' ? { id: `artist:${item.id}`, artist: item } :
            type === 'album' ? { ...item, id: `album:${item.id}`, album: item } : item;
        if (type === 'track') {
            const signal = this._trackEntry(track);
    ...
    recordSearch(query, results) {
        ...
        if (normalized.length < 2 || (this._lastSearch?.query === normalized && Date.now() - this._lastSearch.at < 60_000)) return;
        this._lastSearch = { query: normalized, at: Date.now() };
        // Only close matches express useful intent; unrelated broad results do not.
        for (const [key, type, label] of [['tracks', 'track', 'title'], ['artists', 'artist', 'name']]) {
            const items = Array.isArray(results?.[key]) ? results[key] : results?.[key]?.items || [];
            const matches = items.filter((item) => String(item[label] || '').toLowerCase().startsWith(normalized)).slice(0, 3);
            for (const item of matches) this.recordSignal('search', item, { type });
        }
    }
    _notify(trackId) {
        if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('listening-data-updated', {
    ...
    getTrackSignal(id) { return this._load().tracks[String(id)] || null; }
    getTrackScore(id) {
        const s = this.getTrackSignal(id);
        if (!s) return 0;
        const plays = Math.max(1, s.playCount);
        return s.avgCompletionRatio * 2 + s.completionCount / plays * 3 - (s.earlySkipCount ?? s.skipCount) / plays * 4 +
            (s.likeCount || 0) * 2 + Math.log2(1 + (s.replayCount || 0)) + Math.min(2, (s.playlistAddCount || 0) * 0.3) + Math.min(1, (s.searchCount || 0) * 0.2);
    }
    getArtistSignal(id) { return this._load().artists[String(id).toLowerCase()] || null; }
    getArtistAffinity(id) { const s = this.getArtistSignal(id); return s ? decay(s.affinity, s.updatedAt) : 0; }
    getTopArtists(limit = 20) {
        return Object.entries(this._load().artists).map(([id, v]) => ({ ...v, id, affinity: decay(v.affinity, v.updatedAt) }))
            .filter((v) => v.affinity > 0).sort((a, b) => b.affinity - a.affinity).slice(0, limit);
    }
    getDislikedArtists(limit = 20) {
        return Object.entries(this._load().artists).map(([id, v]) => ({ ...v, id, affinity: decay(v.affinity, v.updatedAt) }))
            .filter((v) => v.playCount >= 2 && v.affinity < -0.3).sort((a, b) => a.affinity - b.affinity).slice(0, limit);
    }
    getHighlyPlayedTracks(limit = 50) {
        return Object.entries(this._load().tracks).filter(([, v]) => v.playCount >= 2 && v.avgCompletionRatio > 0.6)
            .sort((a, b) => b[1].playCount - a[1].playCount).slice(0, limit).map(([id]) => id);
    }
    getFrequentlySkippedTrackIds(limit = 50) {
        return Object.entries(this._load().tracks).filter(([, v]) => v.playCount >= 2 && (v.earlySkipCount ?? v.skipCount) / v.playCount > 0.5)
            .sort((a, b) => b[1].skipCount - a[1].skipCount).slice(0, limit).map(([id]) => id);
    }
    getShortPlayTrackIds(limit = 50) {
        return Object.entries(this._load().tracks).filter(([, v]) => v.playCount >= 2 && v.avgCompletionRatio < COMPLETION_RATIO_THRESHOLD)
            .sort((a, b) => a[1].avgCompletionRatio - b[1].avgCompletionRatio).slice(0, limit).map(([id]) => id);
    }
    getDislikedArtistIds() { return this.getDislikedArtists(30).map((a) => a.id); }
    getTasteProfile() {
        const data = this._load();
        const affinities = (key) => Object.fromEntries(Object.entries(data[key]).map(([id, v]) => [id, decay(v.affinity, v.updatedAt)]));
        return { tracks: data.tracks, artists: affinities('artists'), genres: affinities('genres'), albums: affinities('albums'), recent: data.recent };
    }
    getSessionSignals() {
        return { currentTrackId: this._currentTrackId, accumulatedPlayTime: this._accumulatedPlayTime, trackDuration: this._trackDuration };
    }
    forceFlush() {
