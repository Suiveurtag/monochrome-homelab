export class MusicDatabase {
    ...
    async open() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('selfhost_tracks')) {
                    const store = db.createObjectStore('selfhost_tracks', { keyPath: 'id' });
                    store.createIndex('uploadedAt', 'uploadedAt', { unique: false });
                    store.createIndex('artistId', 'artist.id', { unique: false });
                    store.createIndex('albumId', 'album.id', { unique: false });
                }
                if (!db.objectStoreNames.contains('selfhost_artists')) {
                    const store = db.createObjectStore('selfhost_artists', { keyPath: 'id' });
                    store.createIndex('name', 'name', { unique: false });
                }
                if (!db.objectStoreNames.contains('selfhost_albums')) {
                    const store = db.createObjectStore('selfhost_albums', { keyPath: 'id' });
    ...
    async putUploadedTrack(track) {
        const entry = {
            ...track,
            isLocal: true,
            uploadedAt: track.uploadedAt || Date.now(),
            updatedAt: Date.now(),
            playCount: Number(track.playCount || 0),
        };
        await this.performTransaction('selfhost_tracks', 'readwrite', (store) => store.put(entry));
        window.dispatchEvent(new CustomEvent('local-library-updated'));
        return entry;
    }

    async getUploadedTracks() {
        const tracks = await this.getAll('selfhost_tracks');
        return tracks.sort((a, b) => (b.uploadedAt || 0) - (a.uploadedAt || 0));
    }

    async getUploadedTrack(id) {
        return this.performTransaction('selfhost_tracks', 'readonly', (store) => store.get(id));
    }

    async deleteUploadedTrack(id) {
        await this.performTransaction('selfhost_tracks', 'readwrite', (store) => store.delete(id));
        window.dispatchEvent(new CustomEvent('local-library-updated'));
    }

    async putLocalArtist(artist) {
        const entry = { ...artist, updatedAt: Date.now() };
        await this.performTransaction('selfhost_artists', 'readwrite', (store) => store.put(entry));
        window.dispatchEvent(new CustomEvent('local-library-updated'));
        return entry;
    }

    async getLocalArtists() {
        const artists = await this.getAll('selfhost_artists');
        return artists.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
    }

    async putLocalAlbum(album) {
        const entry = { ...album, updatedAt: Date.now() };
        await this.performTransaction('selfhost_albums', 'readwrite', (store) => store.put(entry));
        window.dispatchEvent(new CustomEvent('local-library-updated'));
        return entry;
    }

    async getLocalAlbums() {
        const albums = await this.getAll('selfhost_albums');
            ...
            return {
                ...
                album: item.album
                    ? {
                          ...
                          videoCoverUrl: item.album.videoCoverUrl || null,
