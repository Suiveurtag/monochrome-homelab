...
test.each(['playlist', 'userplaylist'])('remote edits refresh the active %s route and preserve search and focus', async (route) => {
    vi.useFakeTimers();
    history.replaceState(null, '', `/${route}/shared-mix`);
    document.body.innerHTML = '<main class="main-content"><section id="page-playlist" class="active"><input id="track-list-search-input" value="piano"></section></main>';
    document.getElementById('track-list-search-input').focus();
    ui = { renderPlaylistPage: vi.fn(async () => {
