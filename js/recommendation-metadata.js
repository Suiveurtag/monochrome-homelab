...
export function artistsOf(track) {
    ...
    const artists = (credited.length ? credited : [track?.album?.artist].filter(Boolean))
        .map((artist) => typeof artist === 'string' ? { name: artist } : artist);
    return [...new Map(artists.map((a) => [String(a.id ?? a.name ?? '').trim().toLowerCase(), a]))]
        .filter(([id]) => id).map(([id, a]) => ({ id, name: a.name || '' }));
}
export function genresOf(track) {
    const values = [track?.genre, track?.genres, track?.album?.genre].flat().filter(Boolean);
    return [...new Set(values.flatMap((g) => String(g.name || g).toLowerCase().split(/[,;]/)).map((g) => g.trim()).filter(Boolean))];
}
export function compactTrack(track) {
    return { id: String(track.id), title: track.title || '', artists: artistsOf(track), genres: genresOf(track),
