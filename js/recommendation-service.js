...
export class RecommendationService {
    constructor({
        ...
        favorites = () => db.getFavorites('track'),
        embeddings = audioEmbeddings,
        shouldHideTrack = (track) => contentBlockingSettings.shouldHideTrack(track),
    ...
    setEmbeddingAdapter(adapter) {
        if (adapter !== null && typeof adapter?.getMany !== 'function')
            throw new TypeError('Embedding adapter must implement getMany');
    ...
    async rank(tracks, { seeds = [], limit = 20, ...options } = {}) {
        ...
        const [favorites, embeddings] = await Promise.all([
            ...
            Promise.race([
                ...
                new Promise((resolve) => {
                    timeout = setTimeout(() => resolve(new Map()), this.embeddingTimeout);
                }),
            ])
                .catch(() => new Map())
        ...
        return rankTracks(visible, {
            ...options,
            seeds,
            limit,
            embeddings: embeddings instanceof Map ? embeddings : new Map(),
            favorites: new Set(favorites.map((t) => String(t.id))),
            profile: this.tracker.getTasteProfile(),
        });
    }
}
