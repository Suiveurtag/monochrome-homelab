...
async function initializeSelfHostedUploads() {
    ...
    input.addEventListener('change', async (event) => {
        ...
        const result = await uploadSelfHostedFilesBatch(files, {
            ...
            readTrackMetadata,
            uploadTrack: (metadata, file) => uploadSelfHostedTrack(metadata, file),
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function scanLocalMediaFolder(onlyIfAlreadyScanned = false) {
        ...
        try {
            ...
            UIRenderer.instance.renderLocalFiles(document.getElementById('library-local-container'));
            window.dispatchEvent(new CustomEvent('local-library-updated', { detail: { count: tracks.length } }));
        } finally {
        ...
        if (blob && filename) {
            try {
                ...
                window.dispatchEvent(
                    new CustomEvent('local-library-updated', { detail: { count: window.localFilesCache.length } })
                );
            } catch {
            ...
            if (name) {
                ...
                if (editingId) {
                    ...
                    if (isYTMActive && ytmUrlInput.value.trim()) {
                        alert(
                            'YouTube Music import is disabled in local-only mode. Import a local CSV/M3U/XSPF/JSPF file instead.'
                        );
                        return;
        ...
        if (e.target.closest('#play-playlist-btn')) {
            ...
            try {
                ...
                if (userPlaylist) {
                    ...
                    const { tracks: localTracks } = await MusicAPI.instance.getPlaylist(playlistId);
                    tracks = localTracks;
                }
                if (tracks.length > 0) {
