...
async function coverToPersistentUrl(cover) {
    if (!cover || !String(cover).startsWith('blob:')) return cover;
    try {
...
export class UIRenderer {
    ...
    isFullscreenCoverOpen(overlay = document.getElementById('fullscreen-cover-overlay')) {
        return Boolean(overlay && window.getComputedStyle(overlay).display !== 'none');
    }

    clearFullscreenLocation() {
        if (window.location.hash !== '#fullscreen') return;
        const state =
            window.history.state && typeof window.history.state === 'object' ? { ...window.history.state } : null;
        if (state) delete state.fullscreen;
        window.history.replaceState(state, '', `${window.location.pathname}${window.location.search}`);
    }

    async openCurrentTrackFullscreen({ historyMode = 'push' } = {}) {
        const track = this.player?.currentTrack;
        if (!track) return false;
        return this.showFullscreenCover(
            track,
    ...
    async reconcileFullscreenFromLocation() {
        const overlay = document.getElementById('fullscreen-cover-overlay');
        const isOpen = this.isFullscreenCoverOpen(overlay);

        if (window.location.hash === '#fullscreen') {
            if (isOpen) return true;
            if (!this.player?.currentTrack) {
                this.clearFullscreenLocation();
                return false;
            }
            return this.openCurrentTrackFullscreen({ historyMode: 'none' });
        }

        if (isOpen) this.closeFullscreenCover();
        return false;
    }

    async showFullscreenCover(track, nextTrack, lyricsManager, activeElement, { historyMode = 'push' } = {}) {
        const overlay = document.getElementById('fullscreen-cover-overlay');
        ...
        const fullscreenAudioElement =
            activeElement || this.player?.activeElement || document.getElementById('audio-player');
        const mainContent = document.querySelector('.main-content');
        const playerBar = document.querySelector('.now-playing-bar');

        try {
            // Abort any async side-panel renderer before attaching the shared
            // lyrics observer to fullscreen, then remove the side view at once.
            sidePanelManager.cancelPendingRender();
            clearLyricsPanelSync(fullscreenAudioElement, sidePanelManager.panel);
            if (sidePanelManager.panel?.classList.contains('active')) sidePanelManager.close({ immediate: true });

            // Spicy Lyrics measures its host on connection, so fullscreen must
            // participate in layout before metadata and lyrics are mounted.
            ...
            overlay.classList.toggle('fullscreen-cover-no-round', fullscreenCoverNoRoundSettings.isEnabled());
            ...
            if (historyMode === 'push' && window.location.hash !== '#fullscreen') {
                const state =
                    window.history.state && typeof window.history.state === 'object' ? window.history.state : {};
                window.history.pushState(
                    { ...state, fullscreen: true },
                    '',
                    `${window.location.pathname}${window.location.search}#fullscreen`
                );
            }
            ...
            console.error('Failed to open fullscreen cover:', error);
            return false;
        } finally {
    ...
    async dismissFullscreenCover({ animate = true } = {}) {
        const overlay = document.getElementById('fullscreen-cover-overlay');
        const hasFullscreenLocation = window.location.hash === '#fullscreen';
        const ownsHistoryEntry = hasFullscreenLocation && window.history.state?.fullscreen === true;
        const isVisible = this.isFullscreenCoverOpen(overlay);
        if (!overlay) {
            if (hasFullscreenLocation) this.clearFullscreenLocation();
            return false;
        }
        ...
        if (isVisible) this.closeFullscreenCover();

        if (ownsHistoryEntry) {
            window.history.back();
        } else if (hasFullscreenLocation) {
            // A direct #fullscreen load has no in-app entry to return to.
            // Replacing the URL avoids navigating the user outside Monochrome.
            this.clearFullscreenLocation();
        }
        return isVisible || hasFullscreenLocation;
    }

    closeFullscreenCover() {
        const overlay = document.getElementById('fullscreen-cover-overlay');
        if (!overlay) return false;
        this.fullscreenOpenGeneration += 1;
        this.fullscreenRenderController?.abort();
        this.fullscreenRenderController = null;
        const coverImage = document.getElementById('fullscreen-cover-image');
        const lyricsContent = document.getElementById('fullscreen-lyrics-content');
    ...
    async renderLocalFiles(container) {
        ...
        if (uploadedTracks.length > 0) {
            ...
            if (headerDiv) {
                ...
                headerDiv.querySelector('h3').textContent = `Self-hosted Library (${uploadedTracks.length})`;
            }
            if (listContainer) {
                await this.renderListWithTracks(listContainer, uploadedTracks, true);
            }
        } else if (handle) {
    ...
    async uploadSelfHostedFiles(files) {
        ...
        for (const file of flacFiles) {
            ...
            const id = `selfhost-${crypto.randomUUID()}`;
            const artistId = metadata.artist?.id || `local-artist-${crypto.randomUUID()}`;
            const albumId = metadata.album?.id || `local-album-${crypto.randomUUID()}`;
            const cover = await coverToPersistentUrl(metadata.album?.cover);
            const track = {
                ...metadata,
                id,
                type: 'track',
                ...
                album: {
                    ...(metadata.album || {}),
                    id: albumId,
                    title: metadata.album?.title || 'Unknown Album',
                    cover: cover || 'assets/appicon.png',
                    releaseDate: metadata.album?.releaseDate || '',
                    artist: { id: artistId, name: metadata.artist?.name || 'Unknown Artist' },
                },
                mediaMetadata: { tags: ['Self-hosted', 'FLAC'] },
            };

            await db.putUploadedTrack(track);
            await db.putLocalArtist(track.artist);
            await db.putLocalAlbum(track.album);
        }

        showNotification(`${flacFiles.length} FLAC file${flacFiles.length > 1 ? 's' : ''} uploaded.`);
        await this.renderUploadedTracks();
    }

    async renderUploadedTracks() {
        const container = document.getElementById('selfhost-uploaded-tracks');
        const stats = document.getElementById('selfhost-upload-stats');
        if (!container) return;

        if (!authManager.user) {
            if (stats) stats.textContent = 'Sign in with email/password to upload music to this server.';
            container.innerHTML = createPlaceholder('Server uploads are private per account. Sign in to view them.');
            return;
        }

        const tracks = await listSelfHostedTracks().catch((error) => {
            console.warn('[SelfHost] Failed to list server uploads:', error);
            return [];
        });
        if (stats) {
        ...
        if (tracks.length === 0) {
            container.innerHTML = createPlaceholder('Upload FLAC files to populate Home and Search.');
            return;
        }
        ...
        container.innerHTML = tracks
            .map((track, index) => {
                ...
                return base.replace(
                    '<div class="track-item-actions">',
                    `<div class="track-item-actions">
                        <button class="btn-icon upload-delete-track-btn" type="button" data-id="${escapeHtml(track.id)}" title="Delete">${SVG_TRASH(18)}</button>`
                );
            })
            .join('');
    ...
    async renderExplorePage() {
        ...
        try {
            const localAPI = this.api.getAPI();
            ...
            if (container.children.length === 0) {
                container.innerHTML = createPlaceholder('Select a local music folder to explore your library.');
            }
        } catch (e) {
            console.error(e);
            container.innerHTML = createPlaceholder('Failed to load local explore content.');
        }
    }
            ...
            for (const item of items) {
                ...
                if (selector) {
                    ...
                    if (el) {
                        ...
                        if (type === 'playlist' && !item.isUserPlaylist)
    ...
    async renderGenrePage(genreId, genreName) {
        ...
        contentContainer.innerHTML = createPlaceholder('Genres are not available in local-only mode.');
        ...
        if (picksContainer) {
            ...
            try {
                ...
                for (const item of items.slice(0, 18)) {
                    const data = { ...item.data, _lazy: cardsHTML.length >= 6, _isEditorsPick: true };
                    if (item.type === 'album') {
                        cardsHTML.push(this.createAlbumCardHTML(data));
                    } else if (item.type === 'artist') {
                        cardsHTML.push(this.createArtistCardHTML(data));
                    } else if (item.type === 'user-playlist') {
                        cardsHTML.push(this.createUserPlaylistCardHTML(data));
                    }
                    itemsToStore.push({ data, type: item.type });
                }
                ...
                if (cardsHTML.length > 0) {
                    ...
                    for (const item of itemsToStore) {
                        ...
                        if (el) {
                            ...
                            if (type !== 'user-playlist') await this.updateLikeState(el, type, id);
                        }
                    }
                } else {
                    picksContainer.innerHTML = createPlaceholder('Select a local music folder to show local picks.');
                }
            } catch (e) {
                console.error('Failed to load local picks:', e);
                picksContainer.innerHTML = createPlaceholder('Failed to load local picks.');
            }
        }
    }
    ...
    async renderSearchPage(query) {
        ...
        if (podcastsContainer) {
            ...
            const podcastsSection = podcastsContainer.closest('.content-section');
        ...
        try {
            ...
            if (podcastsContainer) podcastsContainer.innerHTML = '';
        } catch (error) {
    ...
    async renderAlbumPage(albumId, provider = null) {
        ...
        try {
            ...
            if (backgroundSettings.isEnabled() && album.cover && !isVideoArtwork(album.cover)) {
    ...
    async renderArtistPage(artistId, provider = null) {
        ...
        try {
            ...
            if (!isVideoArtwork(artistPic160)) await this.extractAndApplyColor(artistPic160);
    ...
    renderApiSettings() {
        ...
        if (container) {
            const count = Array.isArray(window.localFilesCache) ? window.localFilesCache.length : 0;
            container.innerHTML = `
                <li>
                    <div style="flex: 1; min-width: 0;">
                        <div class="instance-url">Local mode active</div>
                        <div style="font-size: 0.8rem; color: var(--muted-foreground); margin-top: 0.15rem;">
                            ${count} local tracks indexed. Select or reload a music folder from Library > Local Files.
                        </div>
                    </div>
                </li>
            `;
        ...
        if (cacheInfo) {
            cacheInfo.textContent = `Local cache: ${stats.size || 0} entries`;
        }
    }
    ...
    async renderPodcastPage(podcastId) {
        ...
        metaEl.textContent = '';
        nameEl.textContent = 'Podcasts disabled';
        metaEl.textContent = 'Self-hosted mode';
        if (imageEl) imageEl.src = 'assets/appicon.png';
        episodesContainer.innerHTML = createPlaceholder('Podcast APIs have been removed.');
        document.title = 'Podcasts disabled - Monochrome Music';
    }

    async loadAllPodcastEpisodes() {
        const episodesContainer = document.getElementById('podcasts-episodes-container');
        if (episodesContainer) episodesContainer.innerHTML = createPlaceholder('Podcast APIs have been removed.');
    }
