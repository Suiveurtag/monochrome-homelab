...
vi.mock('./db.js', () => ({
    db: { getPlaylist: vi.fn(async (id) => ({ id, name: 'My playlist', tracks: [] })), getFavorites: async () => [] },
}));
vi.mock('./offline-cache.js', () => ({
    offlineCache: {
        scope: () => 'fixture',
        settings: { autoFavorites: false },
        assertScope: () => {},
        cleanup: async () => {},
        entries: async () => [],
        playlists: async () => [],
...
test('Keep offline downloads the personal playlist opened through the library route', async () => {
    ...
    try {
        ...
        document.body.innerHTML =
            '<section id="settings-tab-downloads"><div class="settings-list"></div></section><button id="download-playlist-btn">Download</button>';
        ...
        await vi.waitFor(() =>
            expect(document.querySelector('[data-offline-feedback]').textContent).toContain(
                '0/0 songs available offline'
            )
        );
    } finally {
        persist?.mockRestore();
