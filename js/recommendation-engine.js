    ...
    for (const a of entries(track.id)) {
        for (const seed of seeds) {
            for (const b of entries(seed.id)) {
                if (a.model === b.model && a.version === b.version) best = Math.max(best, cosineSimilarity(a.vector, b.vector));
            }
        }
    }
...
export function scoreTrack(track, { profile = {}, seeds = [], favorites = new Set(), embeddings, now = Date.now() } = {}) {
...
export function rankTracks(tracks, { limit = tracks.length, excludeIds = [], random = Math.random, exploration = 0.8, ...options } = {}) {
    const excluded = new Set([...excludeIds].map(String));
    const unique = new Map();
    for (const track of tracks) {
        if (track?.id == null || track.isUnavailable || track.isPodcast || track.type === 'video' || excluded.has(String(track.id))) continue;
        if (!unique.has(String(track.id))) unique.set(String(track.id), track);
    }
    const pool = [...unique.values()].map((track) => ({ track, artists: artistsOf(track).map((a) => a.id),
        score: scoreTrack(track, options) - Math.log(-Math.log(Math.max(0.00001, Math.min(0.99999, random())))) * exploration }));
    const result = [];
    ...
    while (pool.length && result.length < limit) {
        ...
        for (let i = 0; i < pool.length; i++) {
            const item = pool[i];
            const penalty = item.artists.reduce((max, id) => Math.max(max, (counts.get(id) || 0) * 1.2 + (recentArtists.includes(id) ? 4 : 0)), 0);
            if (item.score - penalty > bestScore) { best = i; bestScore = item.score - penalty; }
        }
...
export function smartRandom(tracks, { profile = {}, currentTrack = null, favorites = new Set(), random = Math.random, now = Date.now() } = {}) {
    const pending = tracks.map((track, index) => ({ track, index, artists: artistsOf(track).map((a) => a.id) }));
    const recentIds = new Set((profile.recent || []).filter((s) => now - s.at < DAY).slice(-20).map((s) => String(s.id)));
    ...
    let recentArtists = (profile.recent || [])
    ...
    while (pending.length) {
        ...
        const weights = pool.map(({ track }) => {
            ...
            return (favorites.has(String(track.id)) || s.likeCount ? 1.3 : 1) * (0.5 + 1 / Math.sqrt(1 + (s.playCount || 0)));
        });
        let draw = random() * weights.reduce((sum, value) => sum + value, 0);
        let chosen = pool.length - 1;
        for (let i = 0; i < weights.length; i++) { draw -= weights[i]; if (draw < 0) { chosen = i; break; } }
        const item = pool[chosen];
        pending.splice(pending.findIndex((p) => p.index === item.index), 1);
        result.push(item.track);
