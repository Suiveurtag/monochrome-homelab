...
beforeEach(() => {
    recommendationService.rank.mockReset();
});
afterEach(() => {
    vi.restoreAllMocks();
});
...
function fixture() {
    ...
    Object.assign(player, {
        ...
        playbackSequence: 1,
        _queueGeneration: 1,
        _recentlyPlayedIds: ['old'],
        radioSeeds: [],
        api: { getTracks: vi.fn(async () => [{ id: 'new' }, { id: 'seed' }, { id: 'old' }]) },
        showRadioLoading: vi.fn(),
        saveQueueState: vi.fn(async () => {}),
        preloadCache: new Map(),
        audio: { pause: vi.fn() },
        disableRadio: vi.fn(function () {
            this.radioEnabled = false;
        }),
        playNext: vi.fn(async function () {
            this.currentQueueIndex++;
            this.playbackSequence++;
        }),
        addToQueue: vi.fn(async function (tracks) {
...
test('concurrent queue extension shares work and rechecks manually queued tracks', async () => {
    ...
    let finish;
    recommendationService.rank.mockImplementation(
        () =>
            new Promise((resolve) => {
                finish = resolve;
            })
...
test('a finished request cannot advance playback after the user changes tracks', async () => {
    const player = fixture();
    let finish;
    player.extendRecommendationQueue = vi.fn(
        () =>
            new Promise((resolve) => {
                finish = resolve;
            })
