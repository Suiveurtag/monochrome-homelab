 ...
 * @param {MusicAPI} [api=MusicAPI.instance] - The local API instance to use for downloading
...
export async function downloadTrackWithMetadata(
    ...
    const enrichedTrack =
        track.isLocal || track.file
            ? track
            : api.enrichTrack
              ? (await api.enrichTrack(track, { downloadQuality: quality })).enrichedTrack
              : await api.getTrackMetadata(track.id);
