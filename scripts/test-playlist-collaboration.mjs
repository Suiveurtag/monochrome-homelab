...
test('PocketBase collaboration lifecycle, authorization and concurrent revisions', { timeout: 60000 }, async (t) => {
    ...
    const args = [`--dir=${directory}`, `--migrationsDir=${resolve('pb_migrations')}`, `--hooksDir=${resolve('pb_hooks')}`, '--dev=false'];
    let processHandle;
    let log = '';
    try {
        ...
        processHandle.stdout.on('data', (data) => { log = (log + data).slice(-20000); });
        processHandle.stderr.on('data', (data) => { log = (log + data).slice(-20000); });
        let ready = false;
        ...
        const admin = (await request('/api/collections/_superusers/auth-with-password', { method: 'POST', body: { identity: email, password } })).token;
        const users = {};
        for (const name of ['owner', 'editor', 'stranger', 'blocked']) {
            const record = await request('/api/collections/users/records', { token: admin, method: 'POST', body: { email: `${name}@example.test`, password, passwordConfirm: password, name, access_status: 'active', role: 'member', verified: true } });
            const profile = await request('/api/collections/DB_users/records', { token: admin, method: 'POST', body: { firebase_id: record.id, username: name, display_name: name, user_playlists: {} } });
            const token = (await request('/api/collections/users/auth-with-password', { method: 'POST', body: { identity: record.email, password } })).token;
            users[name] = { ...record, token, profile };
        }
        const uuid = randomUUID();
        await request(`/api/collections/DB_users/records/${users.owner.profile.id}`, { token: admin, method: 'PATCH', body: { user_playlists: { [uuid]: { id: uuid, tracks: [] } } } });
        let playlist;
        const operate = (who, operation, { revision = playlist.revision, status = 200 } = {}) => request(`/api/monochrome/playlists/${playlist.id}/operations`, { token: users[who].token, method: 'POST', body: { revision, operation }, status });
        const read = (who, status = 200) => request(`/api/collections/shared_playlists/records/${playlist.id}`, { token: users[who].token, status });

        await t.test('promotes local data, retires its legacy snapshot, and locks direct writes', async () => {
            await request('/api/monochrome/playlists', { method: 'POST', body: { uuid, name: 'Together' }, status: 401 });
            playlist = await request('/api/monochrome/playlists', { token: users.owner.token, method: 'POST', body: { uuid, name: 'Together', tracks: [{ id: 'a' }, { id: 'b' }], isPublic: true } });
            ...
            assert.deepEqual(playlist.identities.map((identity) => identity.role), ['owner']);
            const profile = await request(`/api/collections/DB_users/records/${users.owner.profile.id}`, { token: users.owner.token });
            ...
            await request('/api/collections/shared_playlists/records', { token: users.owner.token, method: 'POST', body: { uuid: randomUUID(), owner: users.owner.id, name: 'Bypass', revision: 1 }, status: 403 });
            await request(`/api/collections/shared_playlists/records/${playlist.id}`, { token: users.owner.token, method: 'PATCH', body: { revision: 99, tracks: [] }, status: 403 });
        ...
        await t.test('adds exact usernames and grants editors only track operations', async () => {
            ...
            assert.deepEqual((await read('editor')).identities.map((identity) => identity.role), ['owner', 'collaborator']);
            for (const operation of [{ type: 'add-member', username: 'stranger' }, { type: 'remove-member', userId: users.owner.id }, { type: 'metadata', name: 'Hijack' }, { type: 'delete' }]) {
            ...
            assert.deepEqual(playlist.tracks.map((track) => track.id), ['a', 'b', 'c']);
        });

        await t.test('concurrent clients cannot overwrite an acknowledged revision', async () => {
            const revision = playlist.revision;
            const responses = await Promise.all(['d', 'e'].map((id, index) => fetch(`${base}/api/monochrome/playlists/${playlist.id}/operations`, {
                method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: users[index ? 'editor' : 'owner'].token },
            ...
            await operate('editor', { type: 'reorder', order: ['track:c', 'track:b', 'track:a'] }, { revision, status: 409 });
            ...
            assert.deepEqual(playlist.tracks.map((track) => `track:${track.id}`), order);
        ...
        await t.test('persists metadata and public mirrors while protecting private membership', async () => {
            playlist = await operate('owner', { type: 'metadata', name: 'Shared mix', description: 'Updated together' });
            assert.equal((await read('editor')).name, 'Shared mix');
            let mirrors = await request(`/api/collections/public_playlists/records?filter=${encodeURIComponent(`uuid="${uuid}"`)}`, { token: users.stranger.token });
            assert.equal(mirrors.items[0].name, 'Shared mix');
            assert.deepEqual(mirrors.items[0].tracks.map((track) => track.id), playlist.tracks.map((track) => track.id));
            assert.equal(mirrors.items[0].members, undefined);
            playlist = await operate('owner', { type: 'metadata', isPublic: false });
            mirrors = await request(`/api/collections/public_playlists/records?filter=${encodeURIComponent(`uuid="${uuid}"`)}`, { token: users.stranger.token });
        ...
        await t.test('revocation, leaving, inactive accounts and deletion enforce access immediately', async () => {
            ...
            await request(`/api/collections/users/records/${users.blocked.id}`, { token: admin, method: 'PATCH', body: { access_status: 'banned' } });
