...
test('Real Random preserves every occurrence and does not mutate the queue', () => {
    ...
    assert.deepEqual(result.map((t) => t.id), ['1', '2', '1']);
    assert.deepEqual(input.map((t) => t.id), ['1', '1', '2']);
});
test('Smart Random delays recently heard songs and separates artists when possible', () => {
    ...
    const result = smartRandom(tracks, { random: () => 0, now: 1000,
        currentTrack: song('current', 'a'), profile: { recent: [{ id: 'old', at: 999 }] } });
...
test('ranking uses seed proximity, exclusions and available embeddings', () => {
    ...
    const embeddings = new Map([['seed', [{ model: 'mert', version: '1', vector: [1, 0] }]],
...
test('upload totals retain unknown sizes rather than silently treating them as measured zero', () => {
    assert.deepEqual(summarizeStorage([{ audioSize: 1000 }, {}, { fileSize: 2500 }]), { count: 3, bytes: 3500, unknown: 1 });
...
test('shortcut conflicts require explicit replacement and reserved keys stay protected', () => {
    ...
    const registry = createKeyboardShortcuts({ getItem: (key) => values.get(key), setItem: (key, value) => values.set(key, value), removeItem: (key) => values.delete(key) }, () => {});
...
test('playlist operations preserve distinct media types and reject stale/incomplete reorder', () => {
    ...
    const added = operations.applyTrackOperation(initial, { type: 'add', tracks: [song('1', 'a'), song('2', 'b')] }, 100);
    assert.equal(added.length, 3);
    assert.throws(() => operations.applyTrackOperation(added, { type: 'reorder', order: ['track:1', 'track:2'] }));
    const reordered = operations.applyTrackOperation(added, { type: 'reorder', order: ['track:2', 'video:1', 'track:1'] });
